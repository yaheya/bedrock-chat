...
按照目標 OIDC 提供者的程序，並記下 OIDC 用戶端 ID 和密鑰的值。後續步驟還需要簽發者 URL。如果設置過程需要重定向 URI，請先輸入虛擬值，這將在部署完成後被替換。
...
5. 按照提示為密鑰命名並進行描述。記下密鑰名稱，因為您將在 CDK 程式碼中使用它（在步驟 3 中使用變數名稱 <YOUR_SECRET_NAME>）。
6. 檢查並儲存密鑰。

...
由 [Amazon Bedrock](https://aws.amazon.com/bedrock/) 驅動的多語言生成式人工智慧平台。
支援聊天、具備知識的自訂機器人（RAG）、透過機器人商店分享機器人，以及使用代理程式進行任務自動化。
...
新增您自己的指令和知識（又稱為 [RAG](https://aws.amazon.com/what-is/retrieval-augmented-generation/)）。機器人可以透過機器人商店市集在應用程式使用者之間共享。自訂機器人還可以發佈為獨立 API（請參閱[詳細資訊](./PUBLISH_API_zh-TW.md)）。
...
您還可以匯入現有的 [Amazon Bedrock 知識庫](https://aws.amazon.com/bedrock/knowledge-bases/)。
...
> [!Important]
> 出於治理原因，只有允許的使用者才能建立自訂機器人。要允許建立自訂機器人，使用者必須是名為 `CreatingBotAllowed` 的群組成員，可以通過管理控制台 > Amazon Cognito 使用者池或 aws cli 設置。請注意，可以通過存取 CloudFormation > BedrockChatStack > 輸出 > `AuthUserPoolIdxxxx` 來查看使用者池 ID。

### 管理功能

API 管理、標記機器人為必要、分析機器人使用情況。[詳細資訊](./ADMINISTRATOR_zh-TW.md)
...
通過使用[代理程式功能](./AGENT_zh-TW.md)，您的聊天機器人可以自動處理更複雜的任務。例如，要回答使用者的問題，代理程式可以從外部工具檢索必要資訊，或將任務分解為多個步驟進行處理。
...
- 在 us-east-1 區域，開啟 [Bedrock 模型存取](https://us-east-1.console.aws.amazon.com/bedrock/home?region=us-east-1#/modelaccess) > `管理模型存取` > 勾選您想使用的所有模型，然後點擊 `儲存變更`。
...
- 透過以下指令進行部署。如果您想要指定部署的版本或需要套用安全策略，請從[可選參數](#optional-parameters)中指定適當的參數。
...
- **--repo-url**：要部署的自訂 Bedrock Chat 儲存庫，如果是分叉或自訂原始碼控制。（預設：https://github.com/aws-samples/bedrock-chat.git）
...
> [!注意]
> 覆蓋值將在 AWS 代碼構建部署期間與現有的 cdk.json 配置合併。指定的覆蓋值將優先於 cdk.json 中的值。
...
> [!重要]
> 如果不設置可選參數，此部署方法允許任何知道 URL 的人註冊。對於生產使用，強烈建議添加 IP 位址限制並停用自我註冊，以減輕安全風險（您可以定義 allowed-signup-email-domains 以限制使用者，使僅來自公司網域的電子郵件地址可以註冊）。在執行 ./bin 時，同時使用 ipv4-ranges 和 ipv6-ranges 進行 IP 位址限制，並使用 disable-self-register 停用自我註冊。

> [!提示]
> 如果 `前端 URL` 未出現或 Bedrock Chat 無法正常工作，可能是最新版本的問題。在這種情況下，請在參數中添加 `--version "v3.0.0"` 並重試部署。

## 架構

這是一個建立在 AWS 受管服務上的架構，消除了基礎設施管理的需求。透過使用 Amazon Bedrock，無需與 AWS 外部的 API 通訊。這使得部署可擴展、可靠且安全的應用程式成為可能。
...
超級簡單的部署使用 [AWS CodeBuild](https://aws.amazon.com/codebuild/) 在內部通過 CDK 執行部署。本節描述直接使用 CDK 進行部署的步驟。

- 請準備 UNIX、Docker 和 Node.js 運行環境。如果沒有，您也可以使用 [Cloud9](https://github.com/aws-samples/cloud9-setup-for-prototyping)

> [!重要]
> 如果在部署期間本地環境存儲空間不足，CDK 引導可能會導致錯誤。如果您在 Cloud9 等環境中運行，建議在部署前擴展實例的卷大小。
...
您可以通過兩種方式定義部署參數：使用 `cdk.json` 或使用類型安全的 `parameter.ts` 文件。
...
1. 在 `parameter.ts` 中按照上面的示例定義您的環境
...
1. **棧命名**：
   ...
   - 但是，自定義機器人棧（`BrChatKbStack*`）和 API 發布棧（`ApiPublishmentStack*`）不會收到環境前綴，因為它們是在運行時動態創建的

2. **資源命名**：
   - 只有一些資源會在其名稱中收到環境前綴（例如，`dev_ddb_export` 表，`dev-FrontendWebAcl`）
   - 大多數資源保持其原始名稱，但通過位於不同的棧中來實現隔離
...
6. **資源隔離**：每個環境都會創建其自己的資源集，允許您在同一 AWS 賬戶中擁有開發、測試和生產環境，而不會發生衝突。
...
> [!注意]
> 現有用戶可以繼續使用 `cdk.json` 而無需任何更改。對於新部署或需要管理多個環境的情況，建議使用 `parameter.ts` 方法。
...
1. 在 `parameter.ts` 中定義您的環境，如上所示
2. 每個環境將具有其自己的資源集，並帶有環境特定的前綴
...
4. **預設環境覆蓋**：如果在 `parameter.ts` 中定義了"預設"環境，它將覆蓋 `cdk.json` 中的設置。要繼續使用 `cdk.json`，請不要在 `parameter.ts` 中定義"預設"環境。

5. **環境要求**：要創建"預設"以外的環境，必須使用 `parameter.ts`。僅使用 `-c envName` 選項是不足的，除非有相應的環境定義。

6. **資源隔離**：每個環境都會創建其自己的資源集，允許您在同一個 AWS 帳戶中擁有開發、測試和生產環境，且不會衝突。
...
此範例支援外部身份提供者。目前支援 [Google](./idp/SET_UP_GOOGLE_zh-TW.md) 和[自定義 OIDC 提供者](./idp/SET_UP_CUSTOM_OIDC_zh-TW.md)。
...
`enableRagReplicas` 是 [cdk.json](./cdk/cdk.json) 中的一個選項，用於控制 RAG 資料庫的副本設置，特別是使用 Amazon OpenSearch Serverless 的知識庫。這也會影響機器人商店資料庫。

- **預設**：true
- **true**：通過啟用額外副本增強可用性，適合生產環境，但會增加成本。
- **false**：通過使用較少副本降低成本，適合開發和測試。

這是一個帳戶/區域級別的設置，影響整個應用程式而非個別機器人。

> [!注意]
> 截至 2024 年 6 月，Amazon OpenSearch Serverless 支援 0.5 OCU，降低小規模工作負載的入門成本。生產部署可以從 2 OCU 開始，而開發/測試工作負載可以使用 1 OCU。OpenSearch Serverless 會根據工作負載需求自動擴展。更多詳情請訪問[公告](https://aws.amazon.com/jp/about-aws/whats-new/2024/06/amazon-opensearch-serverless-entry-cost-half-collection-types/)。

### 配置機器人商店

機器人商店功能允許用戶共享和發現自定義機器人。您可以通過 [cdk.json](./cdk/cdk.json) 中的以下設置配置機器人商店：
...
[跨區域推理](https://docs.aws.amazon.com/bedrock/latest/userguide/inference-profiles-support.html)允許 Amazon Bedrock 動態地在多個 AWS 區域間路由模型推理請求，在高峰需求期間提高吞吐量和彈性。要配置，請編輯 `cdk.json`。
...
[Lambda SnapStart](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html) 改善 Lambda 函數的冷啟動時間，提供更快的響應時間以獲得更好的用戶體驗。另一方面，對於 Python 函數，根據緩存大小有[收費](https://aws.amazon.com/lambda/pricing/#SnapStart_Pricing)，並且[目前在某些區域不可用](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html#snapstart-supported-regions)。要停用 SnapStart，請編輯 `cdk.json`。
...
您可以通過在 [cdk.json](./cdk/cdk.json) 中設置以下參數來為 CloudFront 分佈配置自定義網域：
...
- 在 us-east-1 區域使用 DNS 驗證創建 ACM 證書
...
對於功能增強和其他改進，**在創建 Pull Request 之前，我們非常感謝您能創建一個功能請求 Issue 來討論實施方法和細節。對於錯誤修復和語言翻譯（i18n），可以直接創建 Pull Request。**

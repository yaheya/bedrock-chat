...
이미 이전 버전(~`0.4.x`)의 Bedrock Chat을 사용하고 있다면, 마이그레이션을 위해 다음 단계를 따라야 합니다.

## 왜 해야 하나요?

이 주요 업데이트에는 중요한 보안 업데이트가 포함되어 있습니다.

- 벡터 데이터베이스(즉, Aurora PostgreSQL의 pgvector) 저장소가 이제 암호화되어, 배포 시 교체가 트리거됩니다. 이는 기존 벡터 항목이 삭제됨을 의미합니다.
- 봇 생성 가능한 사용자를 제한하기 위해 `CreatingBotAllowed` Cognito 사용자 그룹을 도입했습니다. 현재 기존 사용자는 이 그룹에 속해 있지 않으므로, 봇 생성 권한을 원하는 경우 수동으로 권한을 부여해야 합니다. 참조: [봇 개인화](../../README.md#bot-personalization)
...
- 배포하려는 브랜치를 가져옵니다. 원하는 브랜치(이 경우 `v1`)로 이동하고 최신 변경 사항을 가져옵니다:
...
- DMS로 항목을 복원하려면 반드시 비밀번호 순환을 비활성화하고 데이터베이스에 액세스할 비밀번호를 메모하세요. 마이그레이션 스크립트([migrate_v0_v1.py](./migrate_v0_v1.py))로 복원하는 경우 비밀번호를 메모할 필요가 없습니다.
